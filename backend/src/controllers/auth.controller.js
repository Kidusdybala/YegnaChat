...
import jwt from "jsonwebtoken"; // Completed import
...
export async function Signup(req, res) {
  ...
  const { email, password, fullName, bio, profilePic, location, nativeLanguage, learningLanguage } = req.body;
  ...
  try {
    ...
    const user = new User({
      ...
      fullName,
      bio: bio || "",
      profilePic: profilePic || randomAvatar, // Use random avatar if none provided
      location: location || "",
    ...
    res.cookie("token", token, { httpOnly: true, maxAge: 7 * 24 * 60 * 60 * 1000 }); // 7 days in ms
